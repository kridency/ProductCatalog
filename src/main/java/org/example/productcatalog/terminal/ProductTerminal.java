...
public class ProductTerminal extends AbstractTerminal<Product> {
    ...
    private ProductTerminal() {
        service = AuditProxyFactory.createAuditedProxy(ProductService.getInstance(), ProductService.class, auditor);
    ...
    private final Consumer<Product> view = value -> System.out.println("-".repeat(120) + "\n"
    ...
    public void print(Product product) {
        ...
        if (product.getItem() != null) view.accept(service.find(product.getItem()));
        else service.findFiltered(product).forEach(view);
    }
}
